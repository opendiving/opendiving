...
- **From the server**: the geocoder and the two species *name registers*, WoRMS and Wikidata, on
  ...
  **Wikimedia Commons is a fourth, and it is not one of those registers.** It is never asked
  ...
  themselves are only ever fetched from `upload.wikimedia.org`: that host is hard-coded and has no
  setting, because it is an SSRF fence and a fence with an environment variable in front of it is
  not a fence.
