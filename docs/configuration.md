...
- **From the server**: the geocoder and the two species *name registers*, WoRMS and Wikidata, on
  ...
  **The geocoder carries its own, `GEOCODER_USER_AGENT`, and Nominatim is stricter about it.**
  ...
  enough. And geocoding is not an enrichment that can quietly drop out: dive-site place search and
  reverse geocoding are the whole of it, so a proxy that rewrites outbound headers, or a generic
  string pasted in, takes both out at once. What is left is exactly what `GEOCODER_URL=""` gives —
  search returns nothing, reverse returns "we could not ask", the API stays healthy — and the only
  trace is a log line naming the exception class, never the 403. The reason to change it on a public
  instance is the one above, and it is **not** that the default is generic: it is not, and it is
  accepted. It identifies the project rather than your deployment, so every OpenDiving instance
  arrives at Nominatim under the same name — leaving its operators no way to tell one from another,
  or to reach the one giving them trouble. Put your own project and contact address in.
